...
public class StatementRegressionTest extends BaseTestCase {
	...
	public void testBug60816() throws Exception {
		try {
			this.stmt.execute("drop procedure if exists test1");
			this.stmt.execute("drop procedure if exists test2");
			this.stmt.execute("drop procedure if exists test3");
			this.stmt.execute("CREATE PROCEDURE test1 (INOUT x INTEGER)BEGIN	SET x = x + 1;END ");
			this.stmt.execute("CREATE PROCEDURE test2 (x INTEGER, OUT y INTEGER)BEGIN	SET y = x + 1;END ");
			this.stmt.execute("CREATE PROCEDURE test3 (INOUT x INTEGER)BEGIN	SET x = 10;END ");
			
			CallableStatement call = this.conn.prepareCall("{ call test.test1(?) }");
			call.setInt(1, 1);
			call.registerOutParameter(1, Types.INTEGER);
			call.execute();
			assertEquals(2, call.getInt(1));

			call = this.conn.prepareCall("{ call test.test2(?, ?) }");
			call.setInt(1, 1);
			...
			call = this.conn.prepareCall("{ call test.test1(?) }");
			call.setNull(1, Types.INTEGER);
			call.registerOutParameter(1, Types.INTEGER);
			call.execute();
			...
			call = this.conn.prepareCall("{ call test.test3(?) }");
			call.setNull(1, Types.INTEGER);
			call.registerOutParameter(1, Types.INTEGER);
			call.execute();
