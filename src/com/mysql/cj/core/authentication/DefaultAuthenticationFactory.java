...
import com.mysql.cj.core.io.MysqlSession;
...
public class DefaultAuthenticationFactory implements AuthenticationFactory {
    ...
    public void init(MysqlConnection conn, Protocol prot) {
        ...
        this.exceptionInterceptor = conn.getExceptionInterceptor();
        this.propertySet = conn.getPropertySet();
        this.session = new MysqlSession(this);
        this.session.init(conn, prot);
        this.protocol = this.session.getProtocol();
    }
    ...
    void doHandshake(String user, String password, String database) {
        ...
        this.session.getSessionState().setServerVersion(ServerVersion.parseVersion(buf.readString("ASCII", getExceptionInterceptor())));
        ...
        this.session.getSessionState().setServerCharsetIndex(buf.readByte() & 0xff);
        ...
        this.session.getSessionState().setServerCapabilities(serverCapabilities);
    ...
    private void checkConfidentiality(AuthenticationPlugin plugin) {
        if (plugin.requiresConfidentiality() && !this.protocol.isSSLEstablished()) {
