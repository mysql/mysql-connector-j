            ...
            if (this.useUsageAdvisor) {
                if (!calledExplicitly) {
                    ...
                    this.eventSink.consumeEvent(new ProfilerEvent(ProfilerEvent.TYPE_WARN,
                            "", //$NON-NLS-1$
                            this.currentCatalog, this.connectionId, this.getId(), -1, System.currentTimeMillis(), 0, Constants.MILLIS_I18N, null,
        ...
        synchronized (checkClosed().getConnectionMutex()) {
            ...
            if (max > maxBuf) {
                throw SQLError.createSQLException(Messages.getString("Statement.13", //$NON-NLS-1$
                        new Object[] { Long.valueOf(maxBuf) }), SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
            }
        ...
        synchronized (checkClosed().getConnectionMutex()) {
            if ((max > MysqlDefs.MAX_ROWS) || (max < 0)) {
                throw SQLError.createSQLException(Messages.getString("Statement.15") + max //$NON-NLS-1$
                        + " > " //$NON-NLS-1$ 
                        + MysqlDefs.MAX_ROWS + ".", SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor()); //$NON-NLS-1$ 
            }
