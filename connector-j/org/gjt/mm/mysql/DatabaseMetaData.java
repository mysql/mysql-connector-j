...
public abstract class DatabaseMetaData
{
    ...
    String table) throws java.sql.SQLException {
        ...
        if (_Conn.getIO().versionMeetsMinimum(3, 23, 0))
        {
            ...
            org.gjt.mm.mysql.ResultSet fkRS = _Conn.execSQL(
            "show table status " + DB_Sub + " like '" +
            table + "'", -1);
		    ...
		    if (tableType != null && tableType.toLowerCase().equals("innodb"))
		    {
                	...
                	if (comment != null) {
                    	...
                    	while (commentTokens.hasMoreTokens()) {
                        	...
                        	tuple[9] = Integer.toString(java.sql.DatabaseMetaData.importedKeySetDefault).getBytes(); // UPDATE_RULE
                        	tuple[10] = Integer.toString(java.sql.DatabaseMetaData.importedKeySetDefault).getBytes(); // DELETE_RULE
    ...
    public java.sql.ResultSet getCrossReference(String primaryCatalog,
    	...
    	if (Driver.trace) {
	  		Object[] args = { primaryCatalog, primarySchema, primaryTable, 
		    ...
		    if (tableType != null && tableType.toLowerCase().equals("innodb"))
		    {
                	...
                	if (comment != null) {
                    	...
                    	while (commentTokens.hasMoreTokens()) {
                        	...
                        	StringTokenizer keyTokens = new StringTokenizer(keys, "() /", false);
                        	byte[][] tuple = new byte[14][];
                        	tuple[4] = (foreignCatalog == null ? null : foreignCatalog.getBytes()); // FKTABLE_CAT
	                        tuple[5] = (foreignSchema == null ? null : foreignSchema.getBytes()); // FKTABLE_SCHEM
            	            ...
            	            tuple[7] = s2b(keyTokens.nextToken()); // FKCOLUMN_NAME
                  	      	dummy = keyTokens.nextToken(); // Skip REFER
                        	tuple[0] = s2b(keyTokens.nextToken()); // PKTABLE_CAT
                        	tuple[1] = (primarySchema == null ? null : primarySchema.getBytes()); // PKTABLE_SCHEM
                        	// Skip foreign key if it doesn't refer to the right table
                        	dummy = keyTokens.nextToken(); // PKTABLE_NAME
