...
class Buffer 
{
    ...
    {
        byte []b=buf; 
        return (b[pos++]&0xff)|((b[pos++]&0xff)<<8)|((b[pos++]&0xff)<<16);
    }
    ...
    final long readLength() 
    {
        ...
        switch(sw) {
        case 251 : return (long) 0;
        case 252 : return (long) readInt();
        case 253 : return (long) readLongInt();
        case 254 : return (long) readLong();
        default  : return (long) sw;
	}
    }
    ...
    final long newReadLength()
    {
        ...
        case 252 : return (long) readInt();
        case 253 : return (long) readLongInt();
        case 254 : return (long) readLongLong(); // changed for 64 bit lengths
        default  : return (long) sw;
	}
    }
    ...
    final boolean isLastDataPacket() 
    {
        ...
        return ((buf_length <= 2) && ((buf[0]&0xff) == 254));
    }
	...
	if ((pos + additional_data) > buf_length) 
        {
            ...
            {
                new_length = buf_length + (int)(additional_data * 1.25);
            }
            
	    if (max_length != NO_LENGTH_LIMIT && 
		(new_length > max_length))
            {
		throw new IllegalArgumentException("Packet is larger than max_allowed_packet from server configuration of " + max_length + " bytes");
	    }
